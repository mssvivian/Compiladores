...
public:
  ...
  public:
    ...
    void for_each( F functor ) const {
      ::for_each( atr.begin(), atr.end(), functor );
    }
...
public:
  ...
  static inline constexpr Var selector( Opr operador, const Var& a, const Var& b ) {
  ...
  template <typename Opr>
  static Var selector( Opr operador, const Var& a ) {
  ...
  bool isObject() const {
    return v.index() == OBJECT;
  }
  
  template <class F>
  void for_each( F functor ) const {
    if( v.index() != OBJECT )
      throw newErro( "Essa variável não é um Object: " + toString() );

    get<OBJECT>( v )->for_each( functor );  
  }
...
inline Var operator && ( Var& a, Var& b ) { return Var::selector( Var::e, a, b ); }
