...
CMD_FUNC : FUNCTION NOME_FUNCAO '(' EMPILHA_TS_FUNC LISTA_PARAMS_OPT ')' 
           {
              // Ação de meio de regra: Declara parâmetros
              ...
              $$.c = codigo_params; // Passa código gerado para a próxima parte
           }
           '{' CMDs '}' DESEMPILHA_TS_FUNC 
           { 
             string definicao_lbl_endereco_funcao = ":" + $2.endereco_funcao;
             
             // $7 contém o código gerado pela ação de meio de regra
  ...
  | ID SETA 
    {
      ...
      // Código para inicializar parâmetro
      vector<string> codigo_params;
      codigo_params += decl.c;
      codigo_params += $1.c;
      codigo_params += "arguments";
    ...
    BLOCO
    {
      ...
      set<string> capturas = detecta_capturas(ids_usados, parametros, locais);
       ...
       // Filtra 'print' se não estiver declarado
...
ECOND : ECOND '?' EOBJ ':' EOBJ
      {
        ...
        $$.c = $1.c + "!" + lbl_false + "?" + $3.c + lbl_end + "#" + 
        define_label(lbl_false) + $5.c + define_label(lbl_end);
      }
      | ATRIB
      ;
